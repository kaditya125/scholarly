...
async function main() {
  ...
  const filePath = 'D:\\LUCENT_GENERAL_ENGLISH_COMPLETE.pdf';
  ...
  const notebookId = 'english-notebook-1';
  ...
  if (!existing) {
    ...
    const notebook: any = {
      ...
      userId: userId,
      owner: userId,
      title: `Lucent's General English Complete`,
      color: 'bg-rose-500',
      createdAt: now,
      updatedAt: now,
      lastOpenedAt: now,
      ...
      editors: [], viewers: [],
      description: `Complete book on General English by Lucent.`,
    };
  ...
  const file: any = { 
    originalname: 'LUCENT_GENERAL_ENGLISH_COMPLETE.pdf', 
