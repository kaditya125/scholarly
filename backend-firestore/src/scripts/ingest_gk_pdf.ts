...
async function main() {
  ...
  const filePath = 'D:\\Lucent-General-Knowledge-GK-English-Medium.pdf';
  ...
  const notebookId = 'gk-notebook-1';
  ...
  if (!existing) {
    ...
    const notebook: any = {
      ...
      userId: userId,
      owner: userId,
      title: `Lucent's General Knowledge (English Medium)`,
      color: 'bg-amber-500',
      createdAt: now,
      updatedAt: now,
      lastOpenedAt: now,
      ...
      editors: [], viewers: [],
      description: `Complete book on General Knowledge by Lucent.`,
    };
  ...
  const file: any = { 
    originalname: 'Lucent-General-Knowledge-GK-English-Medium.pdf', 
