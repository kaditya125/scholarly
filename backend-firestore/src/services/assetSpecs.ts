...
LANGUAGE: write the entire article — titles, intros, headings and body — in the SAME language and script as the chapter text below. A Hindi (Devanagari) chapter gets a Hindi article; a Sanskrit chapter gets Sanskrit; an English chapter gets English. Do not translate the chapter into another language, and do not switch language partway through. Proper nouns and established technical terms may keep their usual form.
The JSON keys below stay in English exactly as written; only the values follow the chapter's language. 
...
- LANGUAGE: write the questions, options, answers and explanations in the SAME language and script as the section text below. A Hindi section gets Hindi questions; an English section gets English questions. Do not translate, and do not mix languages within a set. The JSON keys stay in English; only the values follow the section's language.
