...
const redisErrors_1 = require("./utils/redisErrors");
...
const limiter = (0, express_rate_limit_1.default)({
    ...
    max: env_1.env.NODE_ENV === 'development' ? 5000 : 1000,
...
app.get('/api/public/stats', async (_req, res) => {
    try {
        ...
        const [list, presenceSnap] = await Promise.all([
            firebase_1.auth.listUsers(1000),
            firebase_1.db.collection('presence')
...
app.post('/api/presence/heartbeat', verifyToken, async (req, res) => {
    const uid = req.uid;
    try {
        await firebase_1.db.collection('presence').doc(uid).set({ uid, state: 'online', lastActive: Date.now() }, { merge: true });
...
app.post('/api/presence/offline', verifyToken, async (req, res) => {
    const uid = req.uid;
    try {
        await firebase_1.db.collection('presence').doc(uid).set({ uid, state: 'offline', lastActive: Date.now() }, { merge: true });
        cacheTimestamp = 0;
        return res.json({ ok: true });
    }
    catch (err) {
...
process.on('uncaughtException', (err) => {
     ...
     */
    if ((0, redisErrors_1.isTransientRedisDisconnect)(err)) {
