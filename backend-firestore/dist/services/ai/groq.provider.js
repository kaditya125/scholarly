...
class GroqProvider {
    groq;
    modelName;
    constructor(modelName = env_1.env.GROQ_MODEL || 'openai/gpt-oss-20b') {
    ...
    async generateResponse(history, systemPrompt, opts) {
        (0, env_1.assertAIEnabled)('Groq generateResponse');
        ...
        try {
            ...
            logger_1.logger.warn(`GroqProvider generateResponse failed. Falling back to GeminiProvider...`, {
                error: error.message,
                status: error.status,
                code: error.code,
                traceId: tid
            ...
            try {
                ...
                logger_1.logger.error(`Fallback GeminiProvider also failed:`, { error: geminiError.message, traceId: tid });
                throw error;
            }
        }
    }
    async *generateStreamResponse(history, systemPrompt, opts) {
        (0, env_1.assertAIEnabled)('Groq generateStreamResponse');
        const { systemPromptStr, traceId: tid } = this.normalizeArgs(systemPrompt, opts?.traceId);
        const messages = this.buildMessages(history, systemPromptStr);
        let stream;
        try {
            ...
            logger_1.logger.warn(`GroqProvider generateStreamResponse initialization failed. Falling back to GeminiProvider...`, {
                error: error.message,
                status: error.status,
                traceId: tid
            });
        ...
        try {
            ...
            logger_1.logger.error(`GroqProvider streaming interrupted:`, { error: error.message, traceId: tid });
            throw error;
        }
    }
}
